/* Codex of History v2.1.1 — versioned static bootstrap and forced refresh support */
(() => {
  const app = document.getElementById('app');
  const showBoot = (title, text, isError=false) => {
    app.innerHTML = `<main class="boot-screen ${isError?'boot-error':''}"><div class="boot-mark">C</div><div><div class="eyebrow">Content Engine v2.1.1</div><h1>${title}</h1><p>${text}</p></div></main>`;
  };
  ...
  async function registerImageCache(){
    ...
    try{ await navigator.serviceWorker.register('sw.js?v=2.1.1',{scope:'./'}); }
  ...
  async function boot(){
    try {
      ...
      const version=manifest.version||'2.1.1';
      ...
      const [cardSets,relations,campaign,pools,quizzes,stories,mastery,packs,collection,maps,daily,lessons,eras,campaignCatalog,worldTimeline]=await Promise.all([
        ...
        fetchJson(d.eras,version),fetchJson(d.campaignCatalog,version),fetchJson(d.worldTimeline,version)
      ]);
      const cards=cardSets.flat();
      window.CODEX_MANIFEST=manifest;
      window.CODEX_CONFIG={mastery,packs,collection,maps,daily};
      window.CODEX_WORLD={eras,campaigns:campaignCatalog,timeline:worldTimeline};
