/* Codex of History v1.8 — versioned static bootstrap and forced refresh support */
(() => {
  const app = document.getElementById('app');
  const showBoot = (title, text, isError=false) => {
    app.innerHTML = `<main class="boot-screen ${isError?'boot-error':''}"><div class="boot-mark">C</div><div><div class="eyebrow">Content Engine v1.8</div><h1>${title}</h1><p>${text}</p></div></main>`;
  };
  ...
  async function registerImageCache(){
    ...
    try{ await navigator.serviceWorker.register('sw.js?v=1.8.0',{scope:'./'}); }
  ...
  async function boot(){
    try {
      ...
      const version=manifest.version||'1.8.0';
