...
(() => {
  ...
  const showBoot=(title,text,isError=false)=>{app.innerHTML=`<main class="boot-screen ${isError?'boot-error':''}"><div class="boot-mark"><img src="assets/ui/codex-logo-mark.png" alt="Codex of History"></div><div><div class="eyebrow">Content Engine v4.2.0</div><h1>${title}</h1><p>${text}</p></div></main>`;};
  ...
  async function registerImageCache(){if(!('serviceWorker'in navigator)||location.protocol==='file:')return;try{await navigator.serviceWorker.register('sw.js?v=4.2.0',{scope:'./',updateViaCache:'none'});}catch(error){console.warn('[Codex cache]',error);}}
  async function boot(){
    try{
      await registerImageCache();showBoot('Открываем Codex','Загружаем эпохи, кампании, карточки и учебные системы…');
      const manifest=await fetchJson('data/content-manifest.json');const version=manifest.version||'4.2.0';const d=manifest.datasets;
