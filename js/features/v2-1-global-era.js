...
const WORLD_DATA=typeof CODEX_WORLD!=='undefined'?CODEX_WORLD:{
  ...
  campaigns:[
    {id:'MESOPOTAMIA_DAWN',eraId:'ERA_DAWN',order:1,title:'Месопотамия: первые города',subtitle:'Следующая кампания',period:'ок. 4500–2000 до н. э.',chapterCount:10,releasedChapters:0,status:'NEXT',region:'Месопотамия',description:'Следующая кампания.',chapters:Array.from({length:10},(_,i)=>({number:i+1,title:`Глава ${i+1}`}))},
...
function startWorldCampaign(id){
  ...
  if(c.status!=='PLAYABLE'){showToast(c.status==='NEXT'?'Следующая кампания':'Кампания запланирована',c.status==='NEXT'?'Месопотамия начнётся в следующем большом контентном патче.':`План готов: ${c.chapterCount} глав.`,'◎');return;}
...
function worldScreen(){
 ...
 return shell(`<section class="world-hero reveal"><div><div class="eyebrow">Глобальная история</div><h2>Мир развивается<br><span>параллельными путями.</span></h2><p>Изучай эпохи по времени или выбирай региональные кампании. Рим остаётся доступным, а Месопотамия уже доступна как новая стартовая кампания.</p><div class="hero-actions"><button class="btn" onclick="selectWorldEra('ERA_DAWN')">𒀭 Начать с истоков</button><button class="btn secondary" onclick="startWorldCampaign('ROME_CAMPAIGN')">SPQR Продолжить Рим</button></div></div><div class="world-globe"><div class="world-globe-core"><strong>${ERAS.length}</strong><span>эпох</span></div><i></i><i></i><i></i></div></section><section class="world-stats reveal"><article><b>${WORLD_CAMPAIGNS.length}</b><span>кампаний в плане</span></article><article><b>${globalPublishedChapters()}/${globalTotalChapters()}</b><span>глав опубликовано</span></article><article><b>${CARDS.length}</b><span>карточек сейчас</span></article><article><b>v${appVersion()}</b><span>глобальная система</span></article></section>${tabs}${content}${campaignPreviewPanel(preview)}`);
}
