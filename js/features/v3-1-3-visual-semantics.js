/* Codex of History v3.6.0 — semantic historical image resolver with session-only lazy loading */
(() => {
  ...
  try{
    ...
    if(saved&&saved.version==='3.6.0'&&saved.records&&typeof saved.records==='object'){
  ...
  const saveVisualCache=()=>{
    try{
      ...
      VISUAL_STORAGE?.setItem(CACHE_KEY,JSON.stringify({
        version:'3.6.0',lastRun:stateVisual.lastRun,records:stateVisual.records,rejectedCandidates:stateVisual.rejectedCandidates
      }));
  ...
  async function loadQueries(force=false){
    ...
    const url=new URL(QUERY_PATH,location.href);url.searchParams.set('v',CODEX_MANIFEST?.version||'3.6.0');
