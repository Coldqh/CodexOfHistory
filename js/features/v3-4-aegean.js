/* Codex v6.8.0 — Minoans and Mycenaeans */
(()=>{
  const V='6.8.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.AEGEAN_BRONZE='AEGEAN_BRONZE';
  state.aegeanPhase=state.aegeanPhase||'MINOAN';
  state.aegeanExam=state.aegeanExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='AEGEAN_BRONZE')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.AEGEAN_BRONZE||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='AEGEAN_BRONZE'?'МИНОЙЦЫ И МИКЕНЦЫ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='AEGEAN_BRONZE'?'Архив Эгеиды':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='AEGEAN_BRONZE'?'assets/packs/aegean-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.AEGEAN_BRONZE?.eraLayer?.phases||[];}
