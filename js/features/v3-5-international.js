/* Codex v5.1.0 — international world of the Late Bronze Age */
(()=>{
  const V='5.1.0';
  ...
  state.internationalExam=state.internationalExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='BRONZE_INTERNATIONAL')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.BRONZE_INTERNATIONAL||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='BRONZE_INTERNATIONAL'?'МЕЖДУНАРОДНЫЙ МИР БРОНЗОВОГО ВЕКА':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='BRONZE_INTERNATIONAL'?'Архив великих держав':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='BRONZE_INTERNATIONAL'?'assets/packs/international-bronze-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.BRONZE_INTERNATIONAL?.eraLayer?.phases||[];}
