/* Codex v7.3.0 — Bronze Age collapse and transition */
(()=>{
  const V='7.3.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.BRONZE_COLLAPSE='BRONZE_COLLAPSE';
  state.collapsePhase=state.collapsePhase||'PRESSURE';
  state.collapseExam=state.collapseExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='BRONZE_COLLAPSE')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.BRONZE_COLLAPSE||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='BRONZE_COLLAPSE'?'КАТАСТРОФА БРОНЗОВОГО ВЕКА':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='BRONZE_COLLAPSE'?'Архив катастрофы':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='BRONZE_COLLAPSE'?'assets/packs/bronze-collapse-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.BRONZE_COLLAPSE?.eraLayer?.phases||[];}
