/* Codex v6.0.0 — Assyria and Neo-Babylonian Empire */
(()=>{
  const V='6.0.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.ASSYRIA_BABYLON='ASSYRIA_BABYLON';
  state.assyriaPhase=state.assyriaPhase||'RECOVERY';
  state.assyriaExam=state.assyriaExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='ASSYRIA_BABYLON')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.ASSYRIA_BABYLON||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='ASSYRIA_BABYLON'?'АССИРИЯ И НОВОВАВИЛОНСКОЕ ЦАРСТВО':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='ASSYRIA_BABYLON'?'Архив империй':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='ASSYRIA_BABYLON'?'assets/packs/assyria-babylon-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.ASSYRIA_BABYLON?.eraLayer?.phases||[];}
