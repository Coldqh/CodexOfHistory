/* Codex v6.2.0 — Israel, Judah and the Southern Levant */
(()=>{
  const V='6.2.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.ISRAEL_JUDAH='ISRAEL_JUDAH';
  state.levantPhase=state.levantPhase||'FORMATION';
  state.levantExam=state.levantExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='ISRAEL_JUDAH')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.ISRAEL_JUDAH||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='ISRAEL_JUDAH'?'ИЗРАИЛЬ, ИУДЕЯ И ЛЕВАНТ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='ISRAEL_JUDAH'?'Архив Леванта':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='ISRAEL_JUDAH'?'assets/packs/israel-judah-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.ISRAEL_JUDAH?.eraLayer?.phases||[];}
  ...
  function phaseStrip(){
    return `<section class="assyria-phases reveal">${phases().map(p=>`<button class="${state.levantPhase===p.id?'active':''}" onclick="state.levantPhase='${p.id}';save();render()"><small>${p.date}</small><b>${p.title}</b><i><span style="width:${phaseProgress(p)}%"></span></i></button>`).join('')}</section>`;
  }

  const oldHome=home;
  home=function(){
    syncActiveCampaignRuntime();
    if(activeCampaignId()!=='ISRAEL_JUDAH')return oldHome();
    const m=currentMission(),ch=chapterForMission(m.id),phase=phaseForChapter(ch.number),owned=state.unlocked.filter(id=>card(id)?.campaign==='ISRAEL_JUDAH').length;
