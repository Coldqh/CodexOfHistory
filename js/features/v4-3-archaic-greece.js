/* Codex v6.9.1 — Archaic Greece */
(()=>{
  const V='6.9.1';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.GREECE_ARCHAIC='GREECE_ARCHAIC';
  state.archaicPhase=state.archaicPhase||'RECOVERY';
  state.archaicExam=state.archaicExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='GREECE_ARCHAIC')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.GREECE_ARCHAIC||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='GREECE_ARCHAIC'?'АРХАИЧЕСКАЯ ГРЕЦИЯ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='GREECE_ARCHAIC'?'Архив полисов':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='GREECE_ARCHAIC'?'assets/packs/archaic-greece-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.GREECE_ARCHAIC?.eraLayer?.phases||[];}
