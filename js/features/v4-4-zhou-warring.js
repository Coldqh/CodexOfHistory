/* Codex v8.4.0 — Zhou and Warring States */
(()=>{
  const V='8.4.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.ZHOU_WARRING='ZHOU_WARRING';
  state.zhouPhase=state.zhouPhase||'WESTERN';
  state.zhouExam=state.zhouExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='ZHOU_WARRING')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.ZHOU_WARRING||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='ZHOU_WARRING'?'ЧЖОУ И СРАЖАЮЩИЕСЯ ЦАРСТВА':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='ZHOU_WARRING'?'Архив Чжоу':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='ZHOU_WARRING'?'assets/packs/zhou-warring-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.ZHOU_WARRING?.eraLayer?.phases||[];}
