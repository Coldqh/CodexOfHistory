/* Codex v7.3.0 — Vedic India and early states */
(()=>{
  const V='7.3.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.INDIA_VEDIC='INDIA_VEDIC';
  state.vedicPhase=state.vedicPhase||'EARLY';
  state.vedicExam=state.vedicExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='INDIA_VEDIC')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.INDIA_VEDIC||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='INDIA_VEDIC'?'ВЕДИЙСКАЯ ИНДИЯ И РАННИЕ ГОСУДАРСТВА':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='INDIA_VEDIC'?'Ведийский архив':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='INDIA_VEDIC'?'assets/packs/vedic-india-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.INDIA_VEDIC?.eraLayer?.phases||[];}
