/* Codex v8.7.0 — Achaemenid Persia */
(()=>{
  const V='8.7.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.PERSIA='PERSIA';
  state.persiaPhase=state.persiaPhase||'FOUNDATION';
  state.persiaExam=state.persiaExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='PERSIA')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.PERSIA||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='PERSIA'?'АХЕМЕНИДСКАЯ ПЕРСИЯ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='PERSIA'?'Царский архив':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='PERSIA'?'assets/packs/achaemenid-persia-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.PERSIA?.eraLayer?.phases||[];}
