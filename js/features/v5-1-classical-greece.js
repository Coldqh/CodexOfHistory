/* Codex v8.7.1 — Classical Greece */
(()=>{
  const V='8.7.1';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.GREECE_CLASSICAL='GREECE_CLASSICAL';
  state.classicalGreecePhase=state.classicalGreecePhase||'AFTER_WARS';
  state.classicalGreeceExam=state.classicalGreeceExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='GREECE_CLASSICAL')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.GREECE_CLASSICAL||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='GREECE_CLASSICAL'?'КЛАССИЧЕСКАЯ ГРЕЦИЯ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='GREECE_CLASSICAL'?'Архив полисов':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='GREECE_CLASSICAL'?'assets/packs/classical-greece-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.GREECE_CLASSICAL?.eraLayer?.phases||[];}
