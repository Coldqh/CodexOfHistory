/* Codex v5.2.0 — Alexander the Great */
(()=>{
  const V='5.2.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.ALEXANDER='ALEXANDER';
  state.alexanderPhase=state.alexanderPhase||'MACEDONIAN_START';
  state.alexanderExam=state.alexanderExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='ALEXANDER')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.ALEXANDER||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='ALEXANDER'?'АЛЕКСАНДР МАКЕДОНСКИЙ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='ALEXANDER'?'Архив похода':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='ALEXANDER'?'assets/packs/alexander-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.ALEXANDER?.eraLayer?.phases||[];}
