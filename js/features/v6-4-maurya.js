/* Codex v8.5.0 — India: Buddha, Magadha and Mauryas */
(()=>{
  const V='8.5.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.INDIA_MAURYA='INDIA_MAURYA';
  state.mauryaPhase=state.mauryaPhase||'CITIES_TEACHERS';
  state.mauryaExam=state.mauryaExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='INDIA_MAURYA')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.INDIA_MAURYA||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='INDIA_MAURYA'?'ИНДИЯ И МАУРЬИ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='INDIA_MAURYA'?'Имперский архив':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='INDIA_MAURYA'?'assets/packs/maurya-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.INDIA_MAURYA?.eraLayer?.phases||[];}
