/* Codex v7.2.0 — Migrations and New Kingdoms */
(()=>{
  const V='7.2.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.MIGRATION_KINGDOMS='MIGRATION_KINGDOMS';
  state.migrationPhase=state.migrationPhase||'FRONTIER_CRISIS';
  state.migrationExam=state.migrationExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='MIGRATION_KINGDOMS')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.MIGRATION_KINGDOMS||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='MIGRATION_KINGDOMS'?'ПЕРЕСЕЛЕНИЯ И НОВЫЕ КОРОЛЕВСТВА':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='MIGRATION_KINGDOMS'?'Архив переселений и королевств':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='MIGRATION_KINGDOMS'?'assets/packs/migration-kingdoms-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.MIGRATION_KINGDOMS?.eraLayer?.phases||[];}
