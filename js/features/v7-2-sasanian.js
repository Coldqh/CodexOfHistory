/* Codex v7.3.0 — Sasanian Iran */
(()=>{
  const V='7.3.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.SASANIAN='SASANIAN';
  state.sasanianPhase=state.sasanianPhase||'DYNASTIC_FOUNDATION';
  state.sasanianExam=state.sasanianExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='SASANIAN')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.SASANIAN||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='SASANIAN'?'САСАНИДСКИЙ ИРАН':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='SASANIAN'?'Архив Сасанидского Ирана':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='SASANIAN'?'assets/packs/sasanian-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.SASANIAN?.eraLayer?.phases||[];}
