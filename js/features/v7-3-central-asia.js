/* Codex v8.3.0 — Central Asia after Han */
(()=>{
  const V='8.3.0';
  ...
  state.centralAsiaLateExam=state.centralAsiaLateExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='CENTRAL_ASIA_LATE')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.CENTRAL_ASIA_LATE||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='CENTRAL_ASIA_LATE'?'ЦЕНТРАЛЬНАЯ АЗИЯ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='CENTRAL_ASIA_LATE'?'Архив Центральной Азии':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='CENTRAL_ASIA_LATE'?'assets/packs/central-asia-late-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.CENTRAL_ASIA_LATE?.eraLayer?.phases||[];}
