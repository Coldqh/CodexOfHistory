/* Codex v8.4.0 — India from Kushans to Guptas */
(()=>{
  const V='8.4.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.GUPTA='GUPTA';
  state.indiaGuptaPhase=state.indiaGuptaPhase||'POST_MAURYA';
  state.indiaGuptaExam=state.indiaGuptaExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='GUPTA')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.GUPTA||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='GUPTA'?'ИНДИЯ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='GUPTA'?'Архив Индии Кушанов и Гуптов':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='GUPTA'?'assets/packs/india-gupta-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.GUPTA?.eraLayer?.phases||[];}
