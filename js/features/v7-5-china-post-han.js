/* Codex v7.7.0 — China between Han and Tang */
(()=>{
  const V='7.7.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.CHINA_POST_HAN='CHINA_POST_HAN';
  state.chinaPostHanPhase=state.chinaPostHanPhase||'THREE_JIN';
  state.chinaPostHanExam=state.chinaPostHanExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='CHINA_POST_HAN')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.CHINA_POST_HAN||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='CHINA_POST_HAN'?'КИТАЙ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='CHINA_POST_HAN'?'Архив Китая между Хань и Тан':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='CHINA_POST_HAN'?'assets/packs/china-post-han-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.CHINA_POST_HAN?.eraLayer?.phases||[];}
