/* Codex v8.0.0 — Aksum, Nubia and South Arabia */
(()=>{
  const V='8.0.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.AKSUM_NUBIA_ARABIA='AKSUM_NUBIA_ARABIA';
  state.redSeaPhase=state.redSeaPhase||'RED_SEA_NETWORK';
  state.redSeaExam=state.redSeaExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='AKSUM_NUBIA_ARABIA')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.AKSUM_NUBIA_ARABIA||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='AKSUM_NUBIA_ARABIA'?'КРАСНОЕ МОРЕ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='AKSUM_NUBIA_ARABIA'?'Архив Аксумa, Нубии и Аравии':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='AKSUM_NUBIA_ARABIA'?'assets/packs/aksum-nubia-arabia-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.AKSUM_NUBIA_ARABIA?.eraLayer?.phases||[];}
