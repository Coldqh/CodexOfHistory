/* Codex v8.6.0 — Americas in Late Antiquity */
(()=>{
  const V='8.6.0';
  window.CODEX_VERSION=V;
  V22_CAMPAIGN_CODES.AMERICAS_LATE='AMERICAS_LATE';
  state.americasPhase=state.americasPhase||'PARALLEL_AMERICAS';
  state.americasExam=state.americasExam||{};

  const oldSync=syncActiveCampaignRuntime;
  syncActiveCampaignRuntime=function(){
    oldSync();
    if(activeCampaignId()!=='AMERICAS_LATE')return;
    const first=CAMPAIGN.nodes[0];
    (first?.cards||[]).forEach(id=>{if(!state.unlocked.includes(id))state.unlocked.push(id);});
    CODEX_CONFIG.maps=CODEX_MAPS.AMERICAS_LATE||CODEX_CONFIG.maps;
  };

  const oldNoun=activeCampaignNoun;
  activeCampaignNoun=function(){return activeCampaignId()==='AMERICAS_LATE'?'АМЕРИКИ':oldNoun();};
  const oldPackTitle=activeCampaignPackTitle;
  activeCampaignPackTitle=function(){return activeCampaignId()==='AMERICAS_LATE'?'Архив древних Америк':oldPackTitle();};
  const oldPackCover=activeCampaignPackCover;
  activeCampaignPackCover=function(){return activeCampaignId()==='AMERICAS_LATE'?'assets/packs/americas-late-pack.svg':oldPackCover();};

  function phases(){return CODEX_CAMPAIGNS.AMERICAS_LATE?.eraLayer?.phases||[];}
