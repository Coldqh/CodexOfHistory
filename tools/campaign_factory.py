...
def _paragraphs(chapter: dict[str, Any], topic: tuple[str, str, str, str, str, str], mission_title: str, method_notes: list[str]) -> list[str]:
    ...
    chronology = ", ".join(x[0] for x in chapter["chronology"])
    causes = ", ".join(x.lower() for x in chapter["causes"])
    consequences = ", ".join(x.lower() for x in chapter["consequences"])
    source_title = chapter["source"]["title"]
    return [
        f'Глава «{chapter["title"]}» разбирает тему через политическую организацию, хозяйство, мобильность, войну и материальные свидетельства. {chapter["description"]} Миссия «{mission_title}» выделяет сюжет «{title}»: {subtitle}. Датировка «{date}» задаёт рамку, но сама по себе не объясняет механизм изменений.',
        f'Пространство главы включает {locations}. Эти точки не образуют единую неизменную систему. Между ними лежали степи, горы, пустыни, речные долины и зоны оседлого земледелия. Скорость движения людей и вещей зависела от сезона, животных, запасов воды, безопасности переходов и договорённостей с местными властями.',
        f'Хронологический каркас главы включает {chronology}. Некоторые даты известны по письменным сообщениям, другие задаются археологическими диапазонами. При сравнении нужно отделять событие, позднее описание события и современную реконструкцию. Одно и то же имя в источнике может обозначать широкую группу, политическое объединение или взгляд внешнего автора.',
        f'Причины рассматриваемого процесса: {causes}. Они действовали вместе, но с разной силой в отдельных регионах. Военная победа не отменяла переговоров, обмена, брачных союзов и зависимости от местных посредников. Экономический интерес также не создавал автоматического контроля над всем маршрутом или населением.',
        f'Следствия процесса: {consequences}. Их нельзя свести к одному правителю или одному товару. Изменения затрагивали элиты, военные группы, ремесленников, скотоводов, земледельцев, переводчиков и жителей городских узлов. Часть последствий видна только через погребения, монеты, документы, остатки укреплений и распределение вещей.',
        f'Источник «{source_title}» полезен для проверки терминов, датировок и материального контекста. Он не заменяет весь корпус данных. Музейная коллекция показывает сохранённые предметы, письменный текст отражает жанр и интерес автора, археологическая карта зависит от истории раскопок и публикаций.',
        f'Тема «{title}» должна быть связана с институтами и практикой. Вопрос состоит не только в том, кто формально владел территорией, но и в том, кто собирал ресурсы, сопровождал караваны, разводил животных, охранял проходы, чеканил монету, строил укрепления, передавал сведения и переводил названия между языками.',
        f'Сравнение с соседними обществами требует одинаковых критериев: масштаб власти, способы мобилизации, роль городов, характер границ, доступ к металлам и лошадям, формы налога или дани, устойчивость маршрутов и качество источников. Иначе различия подменяются ярлыками «кочевой», «имперский» или «торговый».',
        f'{method_notes[0]} {method_notes[1]} Эти ограничения не мешают делать выводы, но заставляют указывать степень уверенности и альтернативные объяснения. Хороший итог миссии связывает факт, место, дату, тип свидетельства и предел допустимой реконструкции.',
    ]
