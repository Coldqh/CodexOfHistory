...
const manifest=read('data/content-manifest.json'),d=manifest.datasets;assert.equal(manifest.version,'6.0.0');
...
const queries=read('data/image_queries.json');assert.equal(queries.count,2987);assert.equal(queries.cards.HIT_S_01_08.candidates[0].title,'Hatti');assert.equal(queries.cards.HIT_S_09_03.candidates[0].title,'Battle of Kadesh');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v3-3-hittites.js'),'utf8');for(const token of ["const V='6.0.0'",'HITTITES','hittite-phases','openHittiteExamModule','assets/packs/hittites-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
