...
const queries=read('data/image_queries.json');assert.equal(queries.count,2315);assert.equal(queries.cards.AEG_S_04_01.candidates[0].title,'Linear A');assert.equal(queries.cards.AEG_S_08_01.candidates[0].title,'Linear B');assert.equal(queries.cards.AEG_S_09_02.candidates[0].title,'Ahhiyawa');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v3-4-aegean.js'),'utf8');for(const token of ["const V='4.5.0'",'AEGEAN_BRONZE','aegean-phases','openAegeanExamModule','assets/packs/aegean-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
