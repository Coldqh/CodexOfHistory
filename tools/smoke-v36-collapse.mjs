...
const manifest=read('data/content-manifest.json');assert.equal(manifest.version,'3.7.0');
...
const queries=read('data/image_queries.json');assert.equal(queries.version,'3.7.0');assert.equal(queries.count,1595);assert.equal(Object.keys(queries.cards).length,1595);
const images=read('data/image_manifest.json');assert.equal(images.version,'3.7.0');assert.equal(images.count,1595);assert.equal(images.staticHistoricalImageCount,42);assert.equal(images.projectCoverCount,1553);assert.equal(images.dynamicQueryCount,1553);
...
const runtime=fs.readFileSync(path.join(root,'js/features/v3-6-collapse.js'),'utf8');for(const token of ["const V='3.7.0'",'BRONZE_COLLAPSE','collapse-phases','openCollapseExamModule','assets/packs/bronze-collapse-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
