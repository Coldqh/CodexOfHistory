...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.7.2');assert.equal(queries.count,6063);assert.equal(Object.keys(queries.cards).length,6063);
...
const runtime=fs.readFileSync(path.join(root,'js/features/v3-6-collapse.js'),'utf8');for(const token of ["const V='8.7.2'",'BRONZE_COLLAPSE','collapse-phases','openCollapseExamModule','assets/packs/bronze-collapse-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
