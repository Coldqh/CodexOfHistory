...
const queries=read('data/image_queries.json');assert.equal(queries.version,'4.4.0');assert.equal(queries.count,2195);assert.equal(Object.keys(queries.cards).length,2195);
...
const runtime=fs.readFileSync(path.join(root,'js/features/v3-6-collapse.js'),'utf8');for(const token of ["const V='4.4.0'",'BRONZE_COLLAPSE','collapse-phases','openCollapseExamModule','assets/packs/bronze-collapse-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
