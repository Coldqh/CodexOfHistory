...
const queries=read('data/image_queries.json');assert.equal(queries.version,'7.2.0');assert.equal(queries.count,4239);assert.equal(Object.keys(queries.cards).length,4239);assert.equal(queries.cards.ASB_S_03_01.group,'ASSYRIA_BABYLON');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-0-assyria-babylon.js'),'utf8');for(const token of ["const V='7.2.0'",'ASSYRIA_BABYLON','assyria-phases','openAssyriaExamModule','assets/packs/assyria-babylon-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
