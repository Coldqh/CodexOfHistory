...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.4.0');assert.equal(queries.count,5655);assert.equal(Object.keys(queries.cards).length,5655);assert.equal(queries.cards.ASB_S_03_01.group,'ASSYRIA_BABYLON');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-0-assyria-babylon.js'),'utf8');for(const token of ["const V='8.4.0'",'ASSYRIA_BABYLON','assyria-phases','openAssyriaExamModule','assets/packs/assyria-babylon-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
