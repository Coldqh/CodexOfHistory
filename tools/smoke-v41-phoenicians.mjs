...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.7.0');assert.equal(queries.count,6063);assert.equal(Object.keys(queries.cards).length,6063);assert.equal(queries.cards.PHO_S_03_01.group,'PHOENICIANS');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-1-phoenicians.js'),'utf8');for(const token of ["const V='8.7.0'",'PHOENICIANS','assyria-phases','openPhoeniciaExamModule','assets/packs/phoenicians-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
