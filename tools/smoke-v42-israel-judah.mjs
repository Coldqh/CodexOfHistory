...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.0.0');assert.equal(queries.count,5127);assert.equal(Object.keys(queries.cards).length,5127);assert.equal(queries.cards.LEV_S_03_01.group,'ISRAEL_JUDAH');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-2-israel-judah.js'),'utf8');for(const token of ["const V='8.0.0'",'ISRAEL_JUDAH','assyria-phases','openLevantExamModule','assets/packs/israel-judah-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
