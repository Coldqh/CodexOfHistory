...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.1.0');assert.equal(queries.count,5259);assert.equal(Object.keys(queries.cards).length,5259);assert.equal(queries.cards.ARC_S_03_01.group,'GREECE_ARCHAIC');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-3-archaic-greece.js'),'utf8');for(const token of ["const V='8.1.0'",'GREECE_ARCHAIC','assyria-phases','openArchaicExamModule','assets/packs/archaic-greece-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
