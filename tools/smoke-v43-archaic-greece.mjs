...
const queries=read('data/image_queries.json');assert.equal(queries.version,'7.6.0');assert.equal(queries.count,4767);assert.equal(Object.keys(queries.cards).length,4767);assert.equal(queries.cards.ARC_S_03_01.group,'GREECE_ARCHAIC');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-3-archaic-greece.js'),'utf8');for(const token of ["const V='7.6.0'",'GREECE_ARCHAIC','assyria-phases','openArchaicExamModule','assets/packs/archaic-greece-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
