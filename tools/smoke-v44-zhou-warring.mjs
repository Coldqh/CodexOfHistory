...
const queries=read('data/image_queries.json');assert.equal(queries.version,'6.9.1');assert.equal(queries.count,3843);assert.equal(Object.keys(queries.cards).length,3843);assert.equal(queries.cards.ZHO_S_03_01.group,'ZHOU_WARRING');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-4-zhou-warring.js'),'utf8');for(const token of ["const V='6.9.1'",'ZHOU_WARRING','assyria-phases','openZhouExamModule','assets/packs/zhou-warring-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
