...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.5.0');assert.equal(queries.count,5787);assert.equal(Object.keys(queries.cards).length,5787);assert.equal(queries.cards.VED_S_03_01.group,'INDIA_VEDIC');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-5-vedic-india.js'),'utf8');for(const token of ["const V='8.5.0'",'INDIA_VEDIC','assyria-phases','openVedicExamModule','assets/packs/vedic-india-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
