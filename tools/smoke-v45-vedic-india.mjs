...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.7.2');assert.equal(queries.count,6063);assert.equal(Object.keys(queries.cards).length,6063);assert.equal(queries.cards.VED_S_03_01.group,'INDIA_VEDIC');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v4-5-vedic-india.js'),'utf8');for(const token of ["const V='8.7.2'",'INDIA_VEDIC','assyria-phases','openVedicExamModule','assets/packs/vedic-india-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
