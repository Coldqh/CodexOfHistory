...
const queries=read('data/image_queries.json');assert.equal(queries.version,'6.2.0');assert.equal(queries.count,3052);assert.equal(Object.keys(queries.cards).length,3052);assert.equal(queries.cards.PER_S_05_02.group,'PERSIA');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v5-0-achaemenid-persia.js'),'utf8');for(const token of ["const V='6.2.0'",'PERSIA','assyria-phases','openPersiaExamModule','assets/packs/achaemenid-persia-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
