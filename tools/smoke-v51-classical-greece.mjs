...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.2.0');assert.equal(queries.count,5391);assert.equal(Object.keys(queries.cards).length,5391);assert.equal(queries.cards.CLG_S_02_01.group,'GREECE_CLASSICAL');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v5-1-classical-greece.js'),'utf8');for(const token of ["const V='8.2.0'",'GREECE_CLASSICAL','classicalGreecePhase','openClassicalGreeceExamModule','assets/packs/classical-greece-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
