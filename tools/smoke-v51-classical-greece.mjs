...
const queries=read('data/image_queries.json');assert.equal(queries.version,'7.2.0');assert.equal(queries.count,4239);assert.equal(Object.keys(queries.cards).length,4239);assert.equal(queries.cards.CLG_S_02_01.group,'GREECE_CLASSICAL');
const images=read('data/image_manifest.json');assert.equal(images.version,'7.2.0');assert.equal(images.count,4239);assert.equal(images.staticHistoricalImageCount,42);assert.equal(images.projectCoverCount,4197);assert.equal(images.dynamicQueryCount,4197);
...
const runtime=fs.readFileSync(path.join(root,'js/features/v5-1-classical-greece.js'),'utf8');for(const token of ["const V='7.2.0'",'GREECE_CLASSICAL','classicalGreecePhase','openClassicalGreeceExamModule','assets/packs/classical-greece-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
