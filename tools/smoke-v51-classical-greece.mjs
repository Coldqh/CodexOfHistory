...
const manifest=read('data/content-manifest.json');assert.equal(manifest.version,'8.4.0');
...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.4.0');assert.equal(queries.count,5655);assert.equal(Object.keys(queries.cards).length,5655);assert.equal(queries.cards.CLG_S_02_01.group,'GREECE_CLASSICAL');
const images=read('data/image_manifest.json');assert.equal(images.version,'8.4.0');assert.equal(images.count,5655);assert.equal(images.staticHistoricalImageCount,42);assert.equal(images.projectCoverCount,5613);assert.equal(images.dynamicQueryCount,5613);
...
const runtime=fs.readFileSync(path.join(root,'js/features/v5-1-classical-greece.js'),'utf8');for(const token of ["const V='8.4.0'",'GREECE_CLASSICAL','classicalGreecePhase','openClassicalGreeceExamModule','assets/packs/classical-greece-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
