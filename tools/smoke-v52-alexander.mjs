...
const queries=read('data/image_queries.json');assert.equal(queries.version,'7.7.0');assert.equal(queries.count,4899);assert.equal(Object.keys(queries.cards).length,4899);assert.equal(queries.cards.ALX_S_01_01.group,'ALEXANDER');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v5-2-alexander.js'),'utf8');for(const token of ["const V='7.7.0'",'ALEXANDER','alexanderPhase','openAlexanderExamModule','assets/packs/alexander-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
