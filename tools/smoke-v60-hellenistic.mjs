...
const queries=read('data/image_queries.json');assert.equal(queries.version,'7.4.0');assert.equal(queries.count,4503);assert.equal(Object.keys(queries.cards).length,4503);assert.equal(queries.cards.HEL_S_01_01.group,'HELLENISTIC');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v6-0-hellenistic.js'),'utf8');for(const token of ["const V='7.4.0'",'HELLENISTIC','hellenisticPhase','openHellenisticExamModule','assets/packs/hellenistic-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
