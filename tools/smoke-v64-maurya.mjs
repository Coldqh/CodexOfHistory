...
const queries=read('data/image_queries.json');assert.equal(queries.version,'6.4.0');assert.equal(queries.count,3219);assert.equal(Object.keys(queries.cards).length,3219);assert.equal(queries.cards.MAU_S_01_01.group,'INDIA_MAURYA');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v6-4-maurya.js'),'utf8');for(const token of ["const V='6.4.0'",'INDIA_MAURYA','mauryaPhase','CITIES_TEACHERS','openMauryaExamModule','assets/packs/maurya-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
