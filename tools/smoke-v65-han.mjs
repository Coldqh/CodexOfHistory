...
const queries=read('data/image_queries.json');assert.equal(queries.version,'8.7.0');assert.equal(queries.count,6063);assert.equal(Object.keys(queries.cards).length,6063);assert.equal(queries.cards.HAN_S_01_01.group,'HAN');
...
const runtime=fs.readFileSync(path.join(root,'js/features/v6-5-han.js'),'utf8');for(const token of ["const V='8.7.0'",'HAN','hanPhase','QIN_TRANSITION','openHanExamModule','assets/packs/han-pack.svg'])assert.match(runtime,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
