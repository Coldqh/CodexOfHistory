...
const [relations,campaign,pools,quizzes,stories]=await Promise.all([read(d.relations),read(d.campaign),read(d.pools),read(d.quizzes),read(d.stories)]);
const ids=new Set();
...
console.log(`Codex Content Validator v1.1`);console.log(`Карточки: ${cards.length}; связи: ${relations.length}; миссии: ${campaign.nodes.length}; пулы: ${pools.pools.length}; личные истории: ${Object.keys(stories).length}`);
